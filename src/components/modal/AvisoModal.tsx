...
const AvisoModal: React.FC<ModalProps> = ({ open, onClose, title, description, border, button }) => {
    ...
    <dialog className={modalClass} ref={modalRef}>
      ...
      <div className=" rounded-lg z-10 relative max-w-md w-full" id="modal-content">
        <div className="max-w-xs mx-auto flex flex-col items-center bg-white rounded-lg overflow-hidden shadow-lg">
          <div className={`text-center ${border} w-full text-white px-6 py-2`}>
          ...
          {button && (
            <button onClick={onClose} className=" w-min mb-4 rounded-full">
              {button}
            </button>
          )}
        </div>
